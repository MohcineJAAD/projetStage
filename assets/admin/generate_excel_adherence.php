...
$sql = "
    ...
    WHERE NOT EXISTS (
        ...
        WHERE a.identifier = p.identifier
          AND DATE_FORMAT(p.payment_date, '%Y') = ?
          AND p.type = 'adhesion'
    )
";
...
// Load template
$spreadsheet = IOFactory::load('../sheet/payment_adherence_reminder.xlsx');
...
header('Content-Disposition: attachment;filename="adherents_list.xlsx"');
